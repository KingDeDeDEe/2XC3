...
def tri_pivot_quicksort(L):
    ...
    for element in L:
        if element < pivot1:
            ...
            d.append(element)
    return tri_pivot_quicksort(a) + [pivot1] + tri_pivot_quicksort(b) + [pivot2] + tri_pivot_quicksort(c) + [
        pivot3] + tri_pivot_quicksort(d)
...
def test(function):
    ...
    plt.plot(ns, ts, '.', label='function running time')
    plt.xlabel("size of list")
    plt.ylabel("time (t)")
    plt.title("Performance of function")
    plt.legend(loc="upper left")
    plt.show()
    ts.clear()

test(quicksort_copy)   
test(dual_pivot_quicksort)   
test(tri_pivot_quicksort)
test(quad_pivot_quicksort)
